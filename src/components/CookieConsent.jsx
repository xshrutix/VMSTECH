...
export const CookieConsent = () => {
    ...
    const handleAccept = () => {
        ...
        // Additional logic for handling the acceptance of cookies
    };

    const handleDecline = () => {
        setShowConsent(false);
        // Additional logic for handling the rejection of cookies
    };

    if (!showConsent) {
        return null; // Hide the component if showConsent is false
    }

    return (
        <section style={{ zIndex: 9999, position: 'fixed', bottom: '20px', left: '20px' }} className="max-w-2xl p-4 bg-black border border-gray-200 md:gap-x-4 dark:bg-gray-900 md:flex md:items-center dark:border-gray-700 rounded-2xl">
            ...
            <div className="flex items-center mt-6 gap-x-4 shrink-0 lg:mt-0">
                <button onClick={handleDecline} className="text-xs w-1/2 md:w-auto font-medium bg-gray-800 rounded-lg hover:bg-gray-700 text-white px-4 py-2.5 duration-300 transition-colors focus:outline-none">
                    Decline
                </button>

                <button onClick={handleAccept} className="text-xs w-1/2 md:w-auto font-medium bg-gray-800 rounded-lg hover:bg-gray-700 text-white px-4 py-2.5 duration-300 transition-colors focus:outline-none">
                    Accept
                </button>
            </div>
        </section>
